<template>
    ...
    <div class='method-card' v-if='uploadMethod==="drive-link"'>
      <span class='row'>
        <label class='cell label'>Allow Neuroviewer Google Drive access:</label>
        <input type='button' value='Authorize' class='clear-btn cell' @click='authorizeDriveAPI'/>
      </span>
      <span class='row'>
  ...
  const API_KEY = 'AIzaSyAsThYC2kRd1ndaDHq5iGtUScwyvR8ezq8'; //API KEY HERE
  const CLIENT_ID = '776512614515-4g5ttchlsdlals8uvufjv6gkcsno76hi.apps.googleusercontent.com'; //API CLIENT_ID HERE
  ...
  export default {
    ...
    methods: {
      ...
      gapiLoaded() {
        api.load('client', this.initializeGapiClient());
      },
      ...
      authorizeDriveAPI(){
        api = window.gapi;
        // console.log(api)  //FOR TESTING PURPOSES
        this.gapiLoaded();
        this.gisLoaded();
        ...
        setTimeout(function(){
          if (api.client.getToken() === null) {
            // Prompt the user to select a Google Account and ask for consent to share their data for new sessions
      ...
      async fileDataRetrieval(fileId) {
        ...
        try{
          const requestName = await api.client.drive.files.get({
            // fileId: '1kel_qz0_Rv9h-QyhTw26-MKUiUN7yQZZ', //personal test.swc file on drive
            fileId: fileId,
            fields: 'name',
          ...
          const requestFileContent = await api.client.drive.files.get({
            // fileId: '1kel_qz0_Rv9h-QyhTw26-MKUiUN7yQZZ',
            fileId: fileId,
            alt: 'media'
          }).then(function (response) {
